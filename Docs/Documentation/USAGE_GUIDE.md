...
`Workspace` 与 `session feed` 始终是两个独立面板。累计对话内容超出初始 feed 区域后，session feed 会占用 Workspace 腾出的高度；这个高度只会随内容累积改变，不会因滚轮滚动而改变。先向上滚动可浏览会话历史；到达最早事件后继续向上，独立的 Workspace 卡片会从上方出现。右上角显示 `live` 代表自动跟随最新输出，显示 `scroll N` 代表正在查看历史。

| 快捷键 | 功能 |
|--------|------|
| `PageUp` | 向上滚动 |
| `PageDown` | 向下滚动 |
| `鼠标滚轮` | 上下滚动 session feed；向上后暂停自动跟随 |
| `Ctrl-A` (空输入时) | 跳到顶部 |
| `Ctrl-E` (空输入时) | 跳到底部并恢复自动跟随 |

发送一条新命令也会回到最新输出；在此之前，流式回复和工具事件不会强制把正在阅读的历史拉回底部。
